defmodule ISO8583.Message.MTI do
  @moduledoc ~S"""
  ...
  ## Examples
      ...
      false
  """
  @spec is_valid?(mti :: String.t() | atom()) :: true | false
  def is_valid?(mti) when is_binary(mti) do
    mti = mti |> String.to_atom()

    false in [
      is_nil(@private_common_types[mti]),
      is_nil(@message_types[:"1987"][mti]),
      is_nil(@message_types[:"1993"][mti]),
      is_nil(@message_types[:"2003"][mti])
    ]
  end

  def is_valid?(mti) when is_atom(mti) do
    false in [
      is_nil(@private_common_types[mti]),
      is_nil(@message_types[:"1987"][mti]),
      is_nil(@message_types[:"1993"][mti]),
  ...
  ## Examples
      ...
      nil
  """
  @spec get(mti :: String.t() | atom()) :: String.t() | nil
  def get(mti) when is_binary(mti) do
    mti
    |> String.to_atom()
    |> get_mti()
  end

  def get(mti) when is_atom(mti) do
    mti |> get_mti()
  end

  defp get_mti(mti) do
    cond do
      !is_nil(@private_common_types[mti]) -> @private_common_types[mti]
      !is_nil(@message_types[:"1987"][mti]) -> @message_types[:"1987"][mti]
      !is_nil(@message_types[:"1993"][mti]) -> @message_types[:"1993"][mti]
      !is_nil(@message_types[:"2003"][mti]) -> @message_types[:"2003"][mti]
      true -> nil
    end
  end
end
